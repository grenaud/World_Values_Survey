...
def normalize_wvs_v5_v6(df):
    """
    Minimal harmonization for WVS v5/v6:
      - drop unwanted v6-only columns
      - rename N_REG_NUTS1 -> reg_nuts1
    """
    df = df.copy()

    # --- Drop unwanted columns (safe for v5) ---
    cols_to_drop = [
        "td_ctrlcorr",
        "td_goveff",
        "td_polstab",
...
if VERSION_WVS == 5:
    ...
    output_dir = '/home/gabriel/projects/WVS/WVS_v5/'
else:
    df = pd.read_csv("/home/gabriel/projects/WVS/WVS_Cross-National_Wave_7_csv_v6_0.csv.gz",  compression='gzip', encoding= 'utf8')
    df = normalize_wvs_v5_v6(df)
    output_dir = '/home/gabriel/projects/WVS/WVS_v6/'
...
"CDE": ["65", "Canada (English-speaking)", "CDE", "Anglosphere", "0", "11-02-2020-23-03-2020", "CAPI", "English"],
"CDF": ["66", "Canada (French-speaking)", "CDF", "Latin", "0", "11-02-2020-23-03-2020", "CAPI", "French"],
...
"IND": ["67", "India", "IND", "Indo-Iranian", "4090", "01-01-2023-31-03-2023", "CAPI", "Hindi,English"],
...
"KZK": ["27", "Kazakhstan (Kazakh-speaking)", "KZK", "Turkic", "1276", "01-10-2018-30-11-2018", "PAPI", "Kazakh,Russian"],
"KZR": ["27", "Kazakhstan (Russian-speaking)", "KZR", "Slavic", "1276", "01-10-2018-30-11-2018", "PAPI", "Kazakh,Russian"],
...
"UZB": ["68", "Uzbekistan", "UZB", "Turkic", "1200", "01-06-2022-30-08-2022", "CAPI", "Uzbek,Russian"],    
...
# Maps your analysis/output tags to a human-readable description
DICT_DATASET_TAG2DESC = {
    "unfiltered": "Unfiltered dataframe (includes immigrants).",
    "filtered": "Filtered dataframe based on immigration and language spoken at home.",
    "CANKAZ_filter": "Filtered dataset with split for bilingual countries (CAN and KAZ in our case).",
    "USAPolitics_filter": "Filtered dataset where USA is split by political party voters and CAN into French vs English.",
    "USALat_filter": "Filtered dataset where USA is split by latitude (North, USN vs South, USS) and CAN into French vs English.",
    "SecondLanguageFilter": "Filtered dataset based on the second most spoken language in selected countries.",
    "16-29_Filter": "Filtered dataset: age 16–29 only.",
    "30-49_Filter": "Filtered dataset: age 30–49 only.",
    "50andOver_Filter": "Filtered dataset: age 50 and over only.",
    "male": "Filtered dataset: males only.",
    "female": "Filtered dataset: females only.",
    "16-29_CADUSA": "Filtered CAD/USA split dataset: age 16–29 only.",
    "30-49_CADUSA": "Filtered CAD/USA split dataset: age 30–49 only.",
    "50andOver_CADUSA": "Filtered CAD/USA split dataset: age 50 and over only.",
}
...
def _latex_sanitize(s: str) -> str:
    """
    Make a safe fragment for LaTeX labels:
    - lowercase
    - keep [a-z0-9:_-]
    - collapse runs of invalid chars into '-'
    """
    s = s.lower()
...
def _infer_dataset_key_from_png(output_path_png: str, dataset_dict: dict | None) -> str | None:
    if not dataset_dict:
        return None
    filename = os.path.basename(output_path_png)
    for key in sorted(dataset_dict.keys(), key=len, reverse=True):
        if filename.startswith(key):
            return key
    return None

def save_png_and_tex(
    fig,
    output_path_png: str,
    caption: str,
    label: str,
    VERSION_WVS: int,
    dataset_dict: dict | None = None,
    include_dataset_in_label: bool = True,
):
    # ----------------------------
    # Save PNG only if requested
    # ----------------------------
    if fig is not None:
        fig.savefig(output_path_png, dpi=1200, bbox_inches="tight")

    # ----------------------------
    # Infer dataset prefix/key
    # ----------------------------
    dataset_key = _infer_dataset_key_from_png(output_path_png, dataset_dict)
    dataset_desc = dataset_dict.get(dataset_key, "") if (dataset_dict and dataset_key) else ""

    # Add dataset to caption (optional but usually helpful)
    if dataset_desc:
        caption = f"\\textbf{{Dataset:}} {dataset_desc} {caption}"

    # ----------------------------
    # Make label unique per dataset
    # ----------------------------
    if include_dataset_in_label and dataset_key:
        ds = _latex_sanitize(dataset_key)
        # If label is like "fig:english_proficiency_vs_distance_usa"
        # turn it into "fig:filtered:english_proficiency_vs_distance_usa"
    ...
    # ----------------------------
    wvs_dir = "WVS_v6" if VERSION_WVS == 6 else "WVS_v5"
    png_name = os.path.basename(output_path_png)

    tex_path = output_path_png + ".tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\begin{figure}[htbp]\n")
        f.write("    \\centering\n")
        f.write(f"    \\includegraphics[width=\\linewidth]{{{wvs_dir}/{png_name}}}\n")
...
# save_png_and_tex(
#     output_path_png=output_path,  # base path (used for .tex naming)
#     caption="Map of countries colored by language family.",
#     VERSION_WVS=VERSION_WVS,
# )

save_png_and_tex(
    fig=None,  # do NOT resave the PNG
    output_path_png=output_path,
    caption="Map of countries colored by language family.",
    label="fig:language_families_map",
    VERSION_WVS=VERSION_WVS,
    dataset_dict=DICT_DATASET_TAG2DESC,
)
...
colors = [dict_langfam2color[fam] for fam in labels]
...
version_dir = os.path.join(output_dir, f"WVS_v{VERSION_WVS}")
os.makedirs(version_dir, exist_ok=True)

# Save figure
output_path = os.path.join(version_dir, "language_families_pie_chart.png")
fig.savefig(output_path, dpi=300, bbox_inches="tight")
plt.show()

print("Saved:", output_path)

# Write LaTeX include file (do NOT resave PNG)
save_png_and_tex(
    fig=None,
    output_path_png=output_path,
    caption="Distribution of countries by language family in the World Values Survey.",
    label="fig:language_families_pie",
    VERSION_WVS=VERSION_WVS,
    dataset_dict=DICT_DATASET_TAG2DESC,  # optional but recommended
)
...
latex_table = (
    ...
    "\\hline\n"
    "\\textbf{Key} & \\textbf{Question} & \\textbf{Category} & \\textbf{Included} \\\\\n"
    "\\hline\n"
)
...
for key, values in dict_questions.items():
    ...
    latex_table += (
        f"{key} & {values['question']} & "
        f"{values['category']} & {included} \\\\\n\\hline\n"
    )

latex_table += (
    "\\end{tabular}\n"
    "\\caption{Survey questions, thematic category, and inclusion in the analysis}\n"
...
# Count categories
categories = [dict_questions[q]['category'] for q in dict_questions.keys()]
category_counts = Counter(categories)

# Colors
colors = [dict_category2color[cat] for cat in category_counts.keys()]
...
plt.title("Distribution of survey questions by category")
...
# Detect WVS version (adjust if you store it differently)
wvs_dir = "WVS_v6" if VERSION_WVS == 6 else "WVS_v5"

latex_code = f"""
\\begin{{figure}}[htbp]
    \\centering
    \\includegraphics[width=0.7\\linewidth]{{{wvs_dir}/{output_filename}}}
    \\caption{{Distribution of survey questions by thematic category.}}
    \\label{{fig:questions_categories_pie}}
\\end{{figure}}
""".strip()

latex_path = output_path + ".tex"
with open(latex_path, "w") as f:
    f.write(latex_code)

print("Saved LaTeX include:", latex_path)
...
def plot_eng_correlation(english_proficiency_df, cntry, prefixoutput):
    ...
    try:
        ...
        plt.show()
        output_png = f"{prefixoutput}_english_proficiency_vs_distance_{cntry}.png"

        caption = (
        f"English proficiency score versus cultural distance from "
        f"{dict_countrycode2info[cntry][1]}. The regression line shows the best "
        f"linear fit with a 95\\% confidence interval. Pearson correlation "
        f"coefficient and associated p-value are reported in the legend."
        )

        output_png = f"{prefixoutput}_english_proficiency_vs_distance_{cntry}.png"
        label = f"fig:english_proficiency_vs_distance_{cntry.lower()}"
        save_png_and_tex(
                        fig=None,
...
def create_bar_plot(
    ...
    plt.show()
    # --- NEW: write the .tex include file ---
    output_png = outpath

    caption = (
        f"Euclidean cultural distances from {country_name} "
        f"to all other countries, sorted in ascending order. "
        f"Tick label colors denote language families."
    )

    label = f"fig:distances_sorted_{cntry.lower()}"

    # Do NOT resave the PNG; just emit the .tex.
    # The helper will make the label unique by prefixoutput automatically,
    # as long as dataset_dict maps prefixes to descriptions (optional).
    save_png_and_tex(
        fig=None,
        output_path_png=output_png,
        caption=caption,
        label=label,
        VERSION_WVS=VERSION_WVS,
        dataset_dict=DICT_DATASET_TAG2DESC,   # your prefix->meaning dict (recommended)
        include_dataset_in_label=True,
    )
...
def divisive_questions_all_pairs_verbose(
    ...
    texfile = prefixoutput + "_divisive_all_pairs.tex"
    base_label = os.path.basename(prefixoutput)

    caption = (
        "Most divisive and least different survey questions for all country pairs. "
        "Divisiveness is defined as the question with the largest absolute difference "
        "in means relative to pooled variance (Welch $t$-statistic). "
        "Least different questions show minimal between-country difference."
    )

    label = f"tab:divisive_questions_{base_label}"
    ...
    with open(texfile, "w") as f:
        ...
        f.write("\\toprule\n")
        f.write("A & B & Most divisive question & $t$ & $p$ & Least different question & $t$ & $p$ \\\\\n")
        f.write("\\midrule\n")

        for _, r in result_df.iterrows():
            f.write(
                f"{esc(r.A)} & {esc(r.B)} & {esc(r.most_text)} & "
                f"{r.most_t:.2f} & {r.most_p:.2g} & "
...
def write_country_variance_extremes(df_ext: pd.DataFrame, prefixoutput: str):
    ...
    texfile = prefixoutput + "_question_variance_extremes.tex"
    base_label = os.path.basename(prefixoutput)

    caption = (
        "Survey questions with the highest and lowest within-country variance. "
        "For each country, the most divisive question corresponds to the largest "
        "observed variance across respondents, while the least variable question "
        "indicates strong internal agreement. The number of valid responses ($n$) "
        "used to compute the variance is reported."
    )

    label = f"tab:question_variance_extremes_{base_label}"
    ...
    with open(texfile, "w") as f:
        ...
        f.write("\\toprule\n")
        f.write("Country & Highest-variance question & var & $n$ & "
                "Lowest-variance question & var & $n$ \\\\\n")
        f.write("\\midrule\n")

        for _, r in df_ext.iterrows():
            f.write(
                f"{esc(r['country'])} & {esc(r['highest_var_text'])} & "
                f"{r['highest_var']:.3g} & {int(r['highest_var_n'])} & "
                f"{esc(r['lowest_var_text'])} & "
                f"{r['lowest_var']:.3g} & {int(r['lowest_var_n'])} \\\\\n"
            )
...
#    if d==0:
    if True:    
        ...
        def esc(s):
            return str(s).replace("_", "\\_").replace("&", "\\&")
        import os
        texfile = prefixoutput + "top_5_features_per_language_family.tex"
        base_label = os.path.basename(prefixoutput.rstrip("/"))

        caption = (
            "Top five survey questions most predictive of each language family "
            "according to a Random Forest classifier. Feature importance reflects "
            "the relative contribution of each question to classification accuracy. "
            "For each feature, the mean response within the language family is "
            "compared to the global mean across all countries."
        )

        label = f"tab:top5_rf_features_{base_label}"
...
#    if d==0:
    if True:
        ...
        png_path=prefixoutput+"_rawheatmap.png"
        caption = "HEATMAP."  
        ...
        caption = (
            "Heatmap of mean (country-level) responses across survey questions. "
            "Values are scaled to a common 1--5 range; missing responses are excluded."
        )

        # optional visible caption on the figure itself
        plt.figtext(0.5, 0.01, "HEATMAP.", ha="center", fontsize=10)
        plt.show()

        # --- write tex include file ---
        subfile_name = prefixoutput + "_rawheatmap.png.tex"

        base_label = os.path.basename(prefixoutput.rstrip("/"))
        label = f"fig:heatmap_{base_label}"

        wvs_dir = "WVS_v6" if VERSION_WVS == 6 else "WVS_v5"
        png_basename = os.path.basename(png_path)  # e.g. "unfiltered_heatmap.png"
        tex_graphics_path = f"{wvs_dir}/{png_basename}"

        with open(subfile_name, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("\\centering\n")
            f.write(f"\\includegraphics[width=\\linewidth]{{{tex_graphics_path}}}\n")
            f.write(f"\\caption{{{caption}}}\n")
            f.write(f"\\label{{{label}}}\n")
            f.write("\\end{figure}\n")

        print("Wrote:", png_path)
        print("Wrote:", subfile_name)
        ...
        # caption + label (prefix-specific to avoid collisions)
        caption = (
            "Clustered heatmap (clustermap) of imputed country-level mean responses across questions "
            "(scaled to a common 1--5 range). Row/column dendrograms show similarity structure."
        )
        base_label = os.path.basename(prefixoutput.rstrip("/"))
        label = f"fig:clustermap_imp_{base_label}"

        # WVS directory convention for includegraphics
        wvs_dir = "WVS_v6" if VERSION_WVS == 6 else "WVS_v5"
        png_basename = os.path.basename(png_path)  # e.g. "unfiltered_clustermap_imp.png"
        tex_graphics_path = f"{wvs_dir}/{png_basename}"

        # write LaTeX include file (NO \begin{document})
        subfile_name = prefixoutput + "_clustermap_imp.png.tex"
        with open(subfile_name, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("\\centering\n")
            f.write(f"\\includegraphics[width=\\linewidth]{{{tex_graphics_path}}}\n")
...
#             f.write('\\end{figure}\n')
        ...
        # ---- LaTeX include file ----
        caption = (
            "Clustered heatmap of imputed country-level mean survey responses. "
            "Rows correspond to countries and are colored by language family; "
            "columns correspond to survey questions and are colored by thematic category. "
            "Hierarchical clustering is applied to both dimensions using Euclidean distance."
        )

        base_label = os.path.basename(prefixoutput.rstrip("/"))
        label = f"fig:clustermap_lang_imp_{base_label}"

        wvs_dir = "WVS_v6" if VERSION_WVS == 6 else "WVS_v5"
        png_basename = os.path.basename(png_path)
        tex_graphics_path = f"{wvs_dir}/{png_basename}"

        texfile = prefixoutput + "_clustermaplang_imp.png.tex"
        with open(texfile, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("\\centering\n")
            f.write(f"\\includegraphics[width=\\linewidth]{{{tex_graphics_path}}}\n")
        ...
        # Scatter plot with country colors
        colors = [dict_country2color.get(country, (0, 0, 0)) for country in df_mean_imp.index]
        ax.scatter(X_pca[:, 0], X_pca[:, 1], c=colors, s=40, zorder=2)

        # Axis labels (KEEP THESE)
        ax.set_xlabel("PC1", fontproperties=fm.FontProperties(fname=font_path, size=12))
        ax.set_ylabel("PC2", fontproperties=fm.FontProperties(fname=font_path, size=12))

        # Collect text objects for smart adjustment
        texts = []
        ...
        # Automatically avoid overlaps (labels ↔ labels AND labels ↔ points)
        adjust_text(
            texts,
            ax=ax,
            expand_text=(1.2, 1.2),
        ...
        for lang_fam, color in dict_langfam2color.items():
            h = ax.scatter([], [], c=[color], label=lang_fam)
            handles.append(h)
            labels.append(lang_fam)

        ax.legend(handles, labels, fontsize="small", frameon=False)

        # Title with explained variance
        vr = pca.explained_variance_ratio_
        ax.set_title(
            f"PCA ({vr[0]*100:.2f}% variance explained by PC1, "
            f"{vr[1]*100:.2f}% by PC2)",
            fontproperties=fm.FontProperties(fname=font_path, size=12)
        )

        # Save
        out_png = prefixoutput + "_pca_pc1_pc2_imp.png"
        fig.savefig(out_png, dpi=1200, bbox_inches="tight")
        plt.show()


        base_prefix = os.path.basename(prefixoutput.rstrip("/"))

        # Use the globally defined dictionary
        dataset_desc = DICT_DATASET_TAG2DESC.get(base_prefix, base_prefix)

        texfile = prefixoutput + "_pca_pc1_pc2_imp.png.tex"
        ...
        with open(texfile, "w") as f:
            ...
            f.write(
                ...
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_pca_pc1_pc2_imp.png}}\n"
            )
            f.write(
                "  \\caption{"
                "Principal Component Analysis (PCA) of country-level mean responses. "
                "Each point represents a country and is colored by language family. "
                "The first two principal components capture the dominant axes of "
                "variation in cultural values. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(f"  \\label{{fig:pca_pc1_pc2_{base_prefix.lower()}}}\n")
            f.write("\\end{figure}\n")

        print("Wrote LaTeX:", texfile)
        ...
        # Use globally defined dataset dictionary
        dataset_desc = DICT_DATASET_TAG2DESC.get(base_prefix, base_prefix)

        texfile = prefixoutput + "_BiplotPCA.png.tex"
        ...
        with open(texfile, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"  \\includegraphics[width=\\linewidth]"
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_BiplotPCA.png}}\n"
            )
            f.write(
                "  \\caption{"
                "Principal Component Analysis (PCA) biplot of survey questions. "
                "Each point represents a question loading projected onto the first "
                "two principal components. Colors indicate question categories. "
                "Larger markers and labels highlight the questions with the strongest "
                "positive or negative contributions along PC1 and PC2. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(
                f"  \\label{{fig:pca_biplot_{base_prefix.lower()}}}\n"
            )
            f.write("\\end{figure}\n")
        ...
        texfile = f'{prefixoutput}_biplotPCA_circ.png.tex'
        ...
        with open(texfile, "w", encoding="utf-8") as f:
            ...
            f.write(
                ...
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_biplotPCA_circ.png}}\n"
            )
            f.write(
                "  \\caption{"
                "Circular histogram of PCA loading angles for all survey questions. "
                "Angles are computed from the first two principal components "
                "(PC1, PC2), summarizing the directional contribution of each "
                "question in to the principal component space. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(
                f"  \\label{{fig:pca_circular_all_{base_prefix.lower()}}}\n"
            )
            f.write("\\end{figure}\n")
        ...
        for category in unique_categories:
            ...
            texfile = f'{prefixoutput}_biplotPCA_circ_{category}.png.tex'
            ...
            with open(texfile, "w", encoding="utf-8") as f:
                ...
                f.write(
                    ...
                    "Circular histogram of PCA loading angles for all survey questions. "
                    "Angles are computed from the first two principal components "
                    "(PC1, PC2), summarizing the directional contribution of "
                    f"questions in {category} to the principal component space. "
                    f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                    "}\n"
                )
                f.write(
        ...
        # Data for t-SNE (drop language_family column)
        X = df_mean_imp.iloc[:, :-1]

        # Create dictionary of country -> color (by language family)
        dict_country2color = {}
        for country, info in dict_countrycode2info.items():
            lang_fam = info[3]
            color = dict_langfam2color.get(lang_fam, (0, 0, 0))
            dict_country2color[country] = color

        # Run t-SNE
        tsne = TSNE(n_components=2, random_state=42, init="pca", learning_rate="auto")
        embedded_data = tsne.fit_transform(X.values)
        ...
        ax.scatter(embedded_data[:, 0], embedded_data[:, 1], c=colors, s=40, zorder=2)

        # Axis labels (KEEP THESE)
        ax.set_xlabel("t-SNE 1", fontproperties=fm.FontProperties(fname=font_path, size=12))
        ax.set_ylabel("t-SNE 2", fontproperties=fm.FontProperties(fname=font_path, size=12))
        ...
        for lang_fam, color in dict_langfam2color.items():
            h = ax.scatter([], [], c=[color], label=lang_fam)
            handles.append(h)
            legend_labels.append(lang_fam)
        ax.legend(handles, legend_labels, fontsize="small", frameon=False)

        # Title
        ax.set_title(
            "t-SNE embedding of country-level mean responses (colored by language family)",
            fontproperties=fm.FontProperties(fname=font_path, size=12)
        )

        # Save
        out_png = prefixoutput + "_tsne_imp.png"
        fig.savefig(out_png, dpi=1200, bbox_inches="tight")
        plt.show()

        # -----------------------
        # Write LaTeX include file
        # -----------------------
        base_prefix = os.path.basename(prefixoutput.rstrip("/"))
        dataset_desc = DICT_DATASET_TAG2DESC.get(base_prefix, base_prefix)

        texfile = prefixoutput + "_tsne_imp.png.tex"
        with open(texfile, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("  \\centering\n")
            f.write(
                f"  \\includegraphics[width=\\linewidth]"
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_tsne_imp.png}}\n"
            )
            f.write(
                "  \\caption{"
                "t-SNE embedding of country-level mean responses. "
                "Each point represents a country and is colored by language family. "
                "Nearby points indicate countries with more similar average responses "
                "across the included survey questions. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(f"  \\label{{fig:tsne_imp_{base_prefix.lower()}}}\n")
            f.write("\\end{figure}\n")
        ...
        def plot_triangle(
            ...
            repel=True,
            arrow=True,
            max_labels=None,        # e.g. 40 to label only first 40
            min_separation=0.001,   # helps avoid “stuck” labels; tweak if needed
        ):
            """
            points: (n,3) barycentric rows (should sum to 1 each)
            labels: list/array length n
            colors: list length n (optional)
            """
            ...
            for i, coord in enumerate(cart_coords):
                point_color = colors[i] if colors is not None else "k"
                ax.scatter(coord[0], coord[1], color=point_color, marker="o", s=30, zorder=2)

            texts = []
            if label_points:
                # optionally limit how many labels you draw (often the best readability boost)
                idx = range(len(labels))
                if isinstance(max_labels, int) and max_labels > 0:
                    idx = list(idx)[:max_labels]

                for i in idx:
                    x, y = cart_coords[i]
                    point_color = colors[i] if colors is not None else "k"

                    # Place label initially with a small offset
                    t = ax.text(
                        x, y, str(labels[i]),
                        fontsize=fontsize,
                        color=point_color,
                ...
                if repel and texts:
                    adjust_text(
                        ...
                        avoid_self=True,
                        autoalign="xy",
                        # small extra “nudge” so labels don't settle on top of each other
                        add_objects=None,
                        arrowprops=(dict(arrowstyle="-", lw=0.4, color="gray") if arrow else None),
                    )
            ...
            texfile = prefixoutput + "_nmf_c3_triangle.png.tex"

            with open(texfile, "w", encoding="utf-8") as f:
                f.write("\\begin{figure}[htbp]\n")
                f.write("  \\centering\n")
                f.write(
                    f"  \\includegraphics[width=0.75\\linewidth]"
                    f"{{WVS_v{VERSION_WVS}/{base_prefix}_nmf_triangle_c3_imp.png}}\n"
                )
                f.write(
                    "  \\caption{"
                    "Triangular (barycentric) representation of country-level weights "
                    "from Non-Negative Matrix Factorization (NMF) with $k=3$ components. "
                    "Each vertex corresponds to a pure latent cultural component, and "
                    "each point represents a country positioned according to its "
                    "relative component contributions. Countries are colored by "
                    "language family. "
                    f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                    "}\n"
                )
                f.write(
                    f"  \\label{{fig:nmf_triangle_c3_{base_prefix.lower()}}}\n"
                )
                f.write("\\end{figure}\n")
        ...
        for n_components in range(2, 5):
            ...
            texfile = f"{prefixoutput}_nmf_c{n_components}_imp.png.tex"
            ...
            with open(texfile, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"  \\includegraphics[width=\\linewidth]"
                    f"{{WVS_v{VERSION_WVS}/{base_prefix}_nmf_c{n_components}_imp.png}}\n"
                )
                f.write(
                    "  \\caption{"
                    f"Stacked barplot of Non-Negative Matrix Factorization (NMF) component "
                    f"weights for countries, using $k={n_components}$ components. "
                    "Each bar represents a country and is normalized to sum to one, "
                    "illustrating the relative contribution of latent cultural components. "
                    "Countries are colored by language family. "
                    f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                    "}\n"
                )
                f.write(
                    f"  \\label{{fig:nmf_c{n_components}_{base_prefix.lower()}}}\n"
                )
                f.write("\\end{figure}\n")
        ...
        colors = [dict_country2color.get(country, (0, 0, 0)) for country in X.index]
        ax.scatter(X_umap[:, 0], X_umap[:, 1], c=colors, s=40, zorder=2)

        # Axis labels (KEEP THESE)
        ax.set_xlabel("UMAP 1", fontproperties=fm.FontProperties(fname=font_path, size=12))
        ax.set_ylabel("UMAP 2", fontproperties=fm.FontProperties(fname=font_path, size=12))

        # Add labels with smart adjustment
        texts = []
        for i, label in enumerate(X.index):
            texts.append(
                ax.text(
        ...
        # Title
        ax.set_title(
            "UMAP embedding of country-level mean responses (colored by language family)",
            fontproperties=fm.FontProperties(fname=font_path, size=12)
        )
        ...
        # Save PNG
        # -----------------------
        out_png = prefixoutput + "_umap_1_2_imp.png"
        fig.savefig(out_png, dpi=1200, bbox_inches="tight")
        plt.show()
        ...
        texfile = prefixoutput + "_umap_1_2_imp.png.tex"
        with open(texfile, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("  \\centering\n")
            f.write(
                f"  \\includegraphics[width=\\linewidth]"
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_umap_1_2_imp.png}}\n"
            )
            f.write(
                "  \\caption{"
                "UMAP embedding of country-level mean responses. "
                "Each point represents a country and is colored by language family. "
                "UMAP preserves local neighborhood structure, such that nearby points "
                "correspond to countries with more similar average responses across "
                "the included survey questions. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(f"  \\label{{fig:umap_imp_{base_prefix.lower()}}}\n")
            f.write("\\end{figure}\n")
        ...
        texfile = prefixoutput + "_nmf_pairwise_dendrogram_imp.png.tex"
        ...
        with open(texfile, "w") as f:
            ...
            f.write(
                f"  \\includegraphics[width=\\linewidth]"
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_nmf_pairwise_dendrogram_imp.png}}\n"
            )
            f.write(
                "  \\caption{"
                "Pairwise dendrogram derived from Non-negative Matrix Factorization (NMF) "
                "of country-level mean responses. Rows and columns correspond to countries, "
                "and hierarchical clustering highlights similarities in latent value "
                "profiles inferred by NMF. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(f"  \\label{{fig:nmf_pairwise_dendrogram_{base_prefix.lower()}}}\n")
            f.write("\\end{figure}\n")
        ...
        texfile = prefixoutput + "_eng_pro_corr.png.tex"
        ...
        with open(texfile, "w") as f:
            ...
            f.write(
                f"  \\includegraphics[width=\\linewidth]"
                f"{{WVS_v{VERSION_WVS}/{base_prefix}_eng_pro_corr.png}}\n"
            )
            f.write(
                "  \\caption{"
                "Correlation between English proficiency scores and cultural distance "
                "for each country. Bars show the Pearson correlation coefficient "
                "computed between country-level cultural distance and English proficiency. "
                "Countries are ordered by increasing correlation strength. "
                f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
                "}\n"
            )
            f.write(f"  \\label{{fig:eng_pro_corr_{base_prefix.lower()}}}\n")
            f.write("\\end{figure}\n")
...
def compare2dataframes(df1, df2, outputprefix, title):
    ...
    # Show the plot
    plt.savefig(outputprefix+"_barplot_diff_sorted.png", dpi=1200)
    plt.show()

    base_prefix = os.path.basename(outputprefix.rstrip("/"))
    dataset_desc = DICT_DATASET_TAG2DESC.get(base_prefix, base_prefix)

    texfile = outputprefix + "_barplot_diff_sorted.png.tex"
    ...
    with open(texfile, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"  \\includegraphics[width=\\linewidth]"
            f"{{WVS_v{VERSION_WVS}/{base_prefix}_barplot_diff_sorted.png}}\n"
        )
        f.write(
            "  \\caption{"
            f"Comparison of cultural distances: \\textbf{{{title}}}. "
            "Bars represent country-level Euclidean distances, sorted in ascending order, "
            "with colors indicating language family. "
            f"Results shown for the \\textbf{{{dataset_desc}}} dataset."
            "}\n"
        )
        f.write(
            f"  \\label{{fig:distance_barplot_{base_prefix.lower()}}}\n"
        )
        f.write("\\end{figure}\n")
