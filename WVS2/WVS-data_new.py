...
output_dir = '/home/projects/WVS_project/'
...
output_filename = 'language_families_map.png'
...
def plot_eng_correlation(english_proficiency_df, cntry, prefixoutput):
    ...
    try:
        ...
        ax1.set_title(f'English Proficiency Score vs Distance from {dict_countrycode2info[cntry][1]}')
...
def create_bar_plot(cntry, dist_matrix, dict_countrycode2info, dict_langfam2color,  ci_lower, ci_upper , prefixoutput):
    ...
    return plot_eng_correlation(english_proficiency_df, cntry ,prefixoutput)
...
def process_country(cntr, dist_matrix, dict_countrycode2info, dict_langfam2color, ci_lower, ci_upper, prefixoutput):
    try:
        return create_bar_plot(cntr, dist_matrix, dict_countrycode2info, dict_langfam2color, ci_lower, ci_upper, prefixoutput)
        return f"Successfully processed country: {cntr}"
    except Exception as e:
...
def performAnalyses(df,prefixoutput, d):  
    ...
    if d==0:
        ...
        plt.savefig(prefixoutput+"_clustermap_imp.png", dpi=1200)
        plt.show()
        ...
        plt.savefig(prefixoutput + "_clustermaplang_imp.png", dpi=1200)
        plt.show()
        ...
        # Save and show the figure
        plt.savefig(prefixoutput+"BiplotPCA.png", dpi=1200)
        plt.show()
        ...
        plt.savefig(prefixoutput+"_tsne_imp.png", dpi=1200)
        plt.show()
...
df_means, df_var, df_pwdist=performAnalyses(df,output_dir+"unfiltered", 0)
...
def analyseVariance(df_var_filtered,prefixoutput):
    ...
    # Assuming df_var_filtered is your DataFrame
    df_var_filtered_cpy = df_var_filtered.copy()

    # Create a new column 'average' which is the mean of each row
    df_var_filtered_cpy['average'] = df_var_filtered_cpy.mean(axis=1)

    # Sort DataFrame by 'average' column
    df_var_filtered_cpy = df_var_filtered_cpy.sort_values(by='average', ascending=False)



    # Create a bar plot
    fig, ax = plt.subplots(figsize=(10, 6))
    df_var_filtered_cpy['average'].plot(kind='bar')
    ind = np.arange(df_var_filtered_cpy.shape[0])
    plt.xlabel('Country')
    plt.ylabel('Average variance')
    plt.title('Average variance of all answers per Country')
    # Set the x-axis labels to the index of the DataFrame and rotate them by 90 degrees
    ax.set_xticks(ind)
    ax.set_xticklabels(df_var_filtered_cpy.index, rotation=90)
    ax.set_xticks(ind)
            
        # Color the x-axis labels according to language family color
    for i, tick_label in enumerate(ax.get_xticklabels()):
        # Get the country code from the x-axis label
        country_code = tick_label.get_text()
        if country_code in dict_countrycode2info:
        ...
        # Set the font name of the tick label to the specified font
        tick_label.set_fontname(font_name)
    plt.savefig(prefixoutput+"_average_variance.png", dpi=1200)
    plt.show()
...
analyseVariance(df_var_filtered,output_dir+"filtered")
...
# df_means_ALLimm, df_var_ALLimm, df_pwdist_ALLimm=performAnalyses(df_onlyIMM,output_dir+"ALL_imgrnts", 0)
...
#print(df_SecLang.head(10))
print(df_SecLang.shape)
#print(df_SecLang['Q82_EU'])
print(df_SecLang.columns)
